...
> ✅ **フェーズ1・フェーズ2は実装済み**（2026-09-09）。実装時の設計変更は §10 を参照。
...
### 10.4 残っている確認（本番データが必要）

計画 §7 の手動確認のうち、ローカルでは埋められないもの。

| # | 確認内容 |
|---|---|
| 6 | `/analytics` の重なり数カードが **§2① の表（0件:8.00 / 1件:6.40 / 2件:4.47 / 3件:4.40 / 4件:4.00）と一致する**こと。一致すれば集計ロジックの正しさが実データで裏付けられる |
| 7 | 相関表の1行目が **n=28 のストレス×余裕**になっていること（現状は n=8 の参考値がトップ） |
| — | デプロイ後に seeder を手動実行すること（§5 フェーズ1末尾のコマンド）。**実行しないと説明文も `is_none` も本番に入らない** |
